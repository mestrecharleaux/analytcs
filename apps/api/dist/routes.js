...
apiRouter.patch("/sites/:siteId/recordings/:sessionId", async (req, res, next) => {
    try {
        ...
        if (input.watched !== undefined)
            patch.recordingWatched = input.watched;
        if (input.favorite !== undefined)
            patch.recordingFavorite = input.favorite;
        const session = await Session.findOneAndUpdate({ siteId: req.params.siteId, sessionId: req.params.sessionId }, patch, { new: true });
        if (!session)
            return res.status(404).json({ error: "Gravação não encontrada." });
...
apiRouter.post("/sites/:siteId/campaigns", async (req, res, next) => {
    try {
        ...
        if (input.endDate && input.endDate < input.startDate)
            return res.status(422).json({ error: "A data final não pode anteceder a inicial." });
