...
export async function collectReplay(payload, req) {
    const site = await authorizedSite(payload.siteKey, req);
    if (!site)
        return { status: 403, body: { error: "Gravação não autorizada." } };
    if (redis.status === "ready") {
        ...
        if (count > 240)
            return { status: 429, body: { error: "Limite de gravação excedido." } };
    }
    const now = new Date();
    ...
    await Session.findOneAndUpdate({ siteId: site._id, sessionId: payload.sessionId }, {
        $setOnInsert: {
            location: {
...
export async function listRecordings(siteId, options) {
    ...
    if (options.browser && options.browser !== "all")
        query["browser.name"] = options.browser;
    if (options.status === "unwatched")
        query.recordingWatched = false;
    if (options.status === "rage")
        query.recordingRageClicks = { $gt: 0 };
...
export async function recordingDetails(siteId, sessionId) {
