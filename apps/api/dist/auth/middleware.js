...
export const requireAuth = async (req, res, next) => {
    try {
        ...
        if (!session || session.userId !== jwt.userId)
            return res.status(401).json({ error: "Sessão inválida." });
        req.auth = { ...session, sessionToken: jwt.sessionToken };
        next();
    }
    catch {
...
export function requirePermission(permission) {
    return (req, res, next) => {
        const values = req.auth.permissions;
...
export const requireApiPermission = (req, res, next) => {
    const path = req.path;
    let permission;
    if (/\/sites\/[^/]+\/dashboard$/.test(path) || path.includes("/query-keys/"))
        permission = "analytics:view";
    else if (path.includes("/recordings"))
        permission = req.method === "PATCH" ? "recordings:manage" : "recordings:view";
    else if (path.includes("/campaigns"))
        permission = ["POST", "PATCH", "DELETE"].includes(req.method) ? "campaigns:manage" : "campaigns:view";
