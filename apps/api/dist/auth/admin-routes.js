...
adminRouter.patch("/users/:userId", requirePermission("users:manage"), async (req, res, next) => {
    try {
        ...
        if (!current)
            return res.status(404).json({ error: "Usuário não encontrado." });
        if (current.fixed)
            return res.status(409).json({ error: "Usuários fixos não podem ser alterados." });
        const input = z.object({
...
adminRouter.get("/groups/:groupId", requirePermission("groups:view"), async (req, res, next) => {
    try {
        ...
        const userFilter = { groups: group._id };
        if (query.search) {
            userFilter.$or = [
                { name: { $regex: query.search, $options: "i" } },
                { email: { $regex: query.search, $options: "i" } }
...
adminRouter.patch("/groups/:groupId", requirePermission("groups:manage"), async (req, res, next) => {
    try {
        ...
        if (!current)
            return res.status(404).json({ error: "Grupo não encontrado." });
        if (current.fixed)
            return res.status(409).json({ error: "Grupos fixos não podem ser alterados." });
        const input = z.object({
            name: z.string().min(2).max(120).optional(),
            permissions: z.array(z.enum(permissions)).min(1).optional()
        }).parse(req.body);
        const group = await Group.findByIdAndUpdate(req.params.groupId, { ...input, ...(input.permissions ? { permissions: [...new Set(input.permissions)] } : {}) }, { new: true, runValidators: true }).lean();
