...
export function originMatchesDomains(origin, domains) {
    try {
...
const mongoDomainLookup = async (siteKey) => {
    const site = await Site.findOne({ trackingKey: siteKey }).select("domains").lean();
    return site?.domains || null;
};
export function createCollectorCors(domainLookup = mongoDomainLookup, allowAnyOrigin = config.COLLECTOR_ALLOW_ANY_ORIGIN) {
    return async (req, res, next) => {
        try {
            ...
            if (!allowAnyOrigin) {
                ...
                if (!siteKey)
                    return res.status(403).json({ error: "Chave do site ausente na validação de origem." });
