...
test("infere a origem real da requisição quando API_PUBLIC_URL não foi configurada", () => {
    const req = { protocol: "https", get: () => "analytics.example.com" };
    assert.equal(collectorPublicUrl(req), "https://analytics.example.com");
});
test("gera snippet com coleta e gravação habilitadas", () => {
    assert.equal(integrationScript("https://analytics.example.com", "ak_example"), '<script async src="https://analytics.example.com/tracker.js?v=3" data-site="ak_example" data-recording="on"></script>');
});
