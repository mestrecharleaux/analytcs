...
export function analyticsWindow(start, end, timezone = "America/Sao_Paulo") {
    ...
    if (!startAt.isValid || !endExclusive.isValid)
        throw new Error("Período inválido.");
    if (endExclusive <= startAt)
        throw new Error("A data final deve ser igual ou posterior à inicial.");
    if (endExclusive.diff(startAt, "days").days > 370)
        throw new Error("O período máximo é de 370 dias.");
    return {
