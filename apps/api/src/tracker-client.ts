...
if (siteKey && !window.__akrosPulseLoaded) {
  ...
  for (const name of ["pushState", "replaceState"] as const) {
    ...
    history[name] = function (...args: Parameters<History[typeof name]>) {
  ...
  if (replayAllowed) {
    ...
    record({
      emit(event) {
        queue.push(event);
        if (queue.length >= 45 || Date.now() - lastFlush >= 2_000) void flush();
      },
      maskAllInputs: true,
    ...
    let recentClicks: Array<{ x: number; y: number; at: number }> = [];
    addEventListener(
      "click",
      (event) => {
        const now = Date.now();
        ...
        const closeClicks = recentClicks.filter(
          (click) => Math.hypot(click.x - event.clientX, click.y - event.clientY) < 42
        );
