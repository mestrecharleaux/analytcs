...
redis.on("reconnecting", (delay: number) => logger.warn({ retryInMs: delay }, "Redis reconectando"));
...
export async function connectDataStores() {
  ...
  logger.info(
    { mongodb: { endpoint: safeEndpoint(config.MONGODB_URI), durationMs: Math.round(performance.now() - mongoStartedAt) } },
    "MongoDB conectado"
  );
...
export async function cacheGet<T>(key: string): Promise<T | null> {
  if (redis.status !== "ready") {
    if (detailedLogging) logger.debug({ cache: { operation: "get", key, status: "unavailable" } }, "Cache ignorado");
    return null;
  }
  const value = await redis.get(key).catch(() => null);
  if (detailedLogging) logger.debug({ cache: { operation: "get", key, hit: Boolean(value) } }, "Consulta ao cache");
...
export async function cacheSet(key: string, value: unknown, ttl: number) {
  if (redis.status === "ready") {
    ...
    if (detailedLogging) logger.debug({ cache: { operation: "set", key, ttl } }, "Valor armazenado no cache");
  }
}
...
export async function invalidateSiteCache(siteId: string) {
  ...
  if (detailedLogging) logger.debug({ cache: { operation: "invalidate", siteId } }, "Cache analítico invalidado");
}
