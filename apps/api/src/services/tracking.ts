...
export const validDomain = (hostname: string, domains: string[]) => {
...
export function deviceType(parser: UAParser) {
  const device = parser.getDevice();
  if (device.type === "mobile") return "Celular";
  if (device.type === "tablet") return "Tablet";
  if (device.type === "smarttv") return "TV";
  if (device.type === "wearable") return "Vestível";
...
export async function collectAccess(payload: CollectPayload, req: Request) {
  const site = await Site.findOne({ trackingKey: payload.siteKey }).lean();
  if (!site) return { status: 404, body: { error: "Site não encontrado." } };
  ...
  if (redis.status === "ready") {
    ...
    if (count > 180) return { status: 429, body: { error: "Limite de coleta excedido." } };
  }
  ...
  const params = Object.fromEntries(
    Object.entries(payload.queryParams || {})
      .slice(0, 40)
