...
type PairCount = { _id: string | null; count: number };

const normalized = (rows: PairCount[], fallback = "Desconhecido") =>
  rows.map((row) => ({ name: row._id || fallback, count: row.count }));

function topFlows(views: Array<{ sessionId: string; path: string }>) {
  const sessions = new Map<string, string[]>();
  for (const view of views) {
    const paths = sessions.get(view.sessionId) || [];
    if (paths.at(-1) !== view.path) paths.push(view.path);
...
function queryKeyCounts(views: Array<{ queryParams?: Map<string, string> | Record<string, string> | null }>) {
...
export async function siteDashboard(siteId: string, start: string, end: string) {
...
export async function queryValues(siteId: string, key: string, start: string, end: string) {
  const site = await Site.findById(siteId).lean();
  if (!site) return null;
  const window = analyticsWindow(start, end, site.timezone);
...
export async function campaignDashboard(siteId: string, campaignId: string, start: string, end: string) {
  const campaign = await Campaign.findOne({ _id: campaignId, siteId }).lean();
  const site = await Site.findById(siteId).lean();
  if (!campaign || !site) return null;
  const window = analyticsWindow(start, end, site.timezone);
  const match = {
    siteId: new mongoose.Types.ObjectId(siteId),
